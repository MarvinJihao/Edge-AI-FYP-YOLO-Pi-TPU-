...
def load_gt_labels(label_path, img_shape):
    ...
    Returns a tensor with shape [N, 5]: (cls, x1, y1, x2, y2) in pixel
    coordinates.
    """
    h, w = img_shape[:2]
    labels = []
...
def compute_metrics_per_image(det, gt, iou_thres=0.5):
    """
    Compute TP, FP, and FN counts for one image.

    det: tensor [M, 6] (x1, y1, x2, y2, conf, cls), already mapped to original
    image size.
...
def print_metrics_table(global_metrics, names, iou_thres):
    ...
    print(f"  {'-' * 72}")
    print(
        f"  {'ALL':<20} {all_tp:>6} {all_fp:>6} {all_fn:>6} "
        f"{all_prec:>8.3f} {all_recall:>8.3f} {all_f1:>8.3f} {mAP:>8.3f}  -> mAP"
    )
    print(f"{'=' * 74}\n")
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):
            ...
            label_path = str(p).replace(os.sep + "images" + os.sep, os.sep + "labels" + os.sep)
            label_path = os.path.splitext(label_path)[0] + ".txt"
            gt = load_gt_labels(label_path, im0.shape)
        ...
        LOGGER.info(
            ...
            f"| mem: {mem_after:.1f}MB (delta {mem_delta:+.1f}MB) "
            f"| cpu: {cpu_pct:.1f}%"
        )
    ...
    if all_perf_stats:
        ...
        def avg(key):
            return sum(x[key] for x in all_perf_stats) / n

        t = tuple(x.t / seen * 1e3 for x in dt)
        LOGGER.info(f"Speed: %.1fms pre-process, %.1fms inference, %.1fms NMS per image at shape {(1, 3, *imgsz)}" % t)
        LOGGER.info(
            f"\n{'=' * 60}\n"
            f"  Processed {n} images. Average inference statistics per image:\n"
            f"  Pre-process: {avg('pre_ms'):.1f}ms  Inference: {avg('infer_ms'):.1f}ms  "
